...
static vfs_node_t vfs_node_alloc(vfs_node_t parent, cstr name) {
  ...
  if (parent) list_prepend(parent->info->child, node);
  return node;
}

static void vfs_free(vfs_node_t vfs) {
  if (vfs == null) return;
  list_free_with(vfs->info->child, (free_t)vfs_free);
  vfs_close(vfs);
  free(vfs->name);
  free(vfs->info);
  free(vfs);
...
finline __nnull(1) void do_open(vfs_node_t file) {
  if (file->info->handle != null) {
    callbackof(file, stat)(file->info->handle, file->info);
  } else {
    callbackof(file, open)(file->parent->info->handle, file->name, file->info);
  }
}
...
finline void do_update(vfs_node_t file) {
  ...
  if (file->info->type == file_none || file->info->handle == null || file->info->type == file_dir) do_open(file);
...
static vfs_node_t vfs_child_find(vfs_node_t parent, cstr name) {
  return list_first(parent->info->child, data, streq(name, ((vfs_node_t)data)->name));
}
...
int vfs_mkdir(cstr name) {
  ...
  upd:
    if (current == null) {
      ...
      callbackof(father, mkdir)(father->info->handle, buf, current->info);
    } else {
      do_update(current);
      if (current->info->type != file_dir) goto err;
    }
  }
...
int vfs_mkfile(cstr name) {
  ...
  for (cstr buf = pathtok(&save_ptr); buf; buf = pathtok(&save_ptr)) {
    if (streq(buf, ".")) continue;
    if (streq(buf, "..")) {
      if (!current->parent || current->info->type != file_dir) goto err;
      current = current->parent;
      continue;
    }
    current = vfs_child_find(current, buf);
    if (current == null || current->info->type != file_dir) goto err;
  }

create:
  vfs_node_t node = vfs_child_append(current, filename, null);
  node->info->type      = file_block;
  callbackof(current, mkfile)(current->info->handle, filename, node->info);
...
static vfs_node_t vfs_do_search(vfs_node_t dir, cstr name) {
  return list_first(dir->info->child, data, streq(name, ((vfs_node_t)data)->name));
}
...
int vfs_close(vfs_node_t node) {
  ...
  if (node->info->handle == null) return 0;
...
int vfs_mount(cstr src, vfs_node_t node) {
  if (node == null) return -1;
  if (node->info->type != file_dir) return -1;
  for (int i = 1; i < fs_nextid; i++) {
    if (fs_callbacks[i]->mount(src, node->info) == 0) {
      node->info->fsid = i;
      node->info->root = node;
      return 0;
    }
...
ssize_t vfs_read(vfs_node_t file, void *addr, size_t offset, size_t size) {
  ...
  assert(addr != null);
  do_update(file);
  if (file->info->type == file_dir) return -1;
...
ssize_t vfs_write(vfs_node_t file, const void *addr, size_t offset, size_t size) {
  ...
  do_update(file);
  if (file->info->type == file_dir) return -1;
  ssize_t write_bytes = callbackof(file, write)(file->info->handle, addr, offset, size);
  if (write_bytes > 0) { file->info->size = max(file->info->size, offset + write_bytes); }
  return write_bytes;
}
...
int vfs_unmount(cstr path) {
  ...
  if (node->parent) {
    ...
    if (cur->info->root == cur) {
      ...
      if (cur->info->fsid) do_update(cur);
      return 0;
    }
  }
  return -1;
